...
static bool isCSRestore(MachineInstr *MI, const MCPhysReg *CSRegs) {
  if (IdempotenceConstructionMode != IdempotenceOptions::NoConstruction &&
      MI->getOpcode() == ARM::tLDMIA)
  {
    // FIXME: Add some checking to make sure this is the correct LDMIA
    return true;  
  }
...
void Thumb1FrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  if (ArgRegsSaveSize || IsV4PopReturn) {
    ...
    if (IdempotenceConstructionMode != IdempotenceOptions::NoConstruction)
    {
      // Find the callee reg restoring function
      while (MBBI != MBB.begin() && !isCSRestore(MBBI, CSRegs))
        MBBI--;

      //if (!AFI->shouldRestoreSPFromFP()) {
        emitThumbRegPlusImmediate(MBB, MBBI, dl, ARM::R3, ARM::SP, NumBytes,
                                TII, *RegInfo);
      //}else{
      //  // Reset SP based on frame pointer only if the stack frame extends beyond
      //  // frame pointer stack slot, the target is ELF and the function has FP, or
      ...
      // If there are no CS regs, make sure we have the SP handy
      unsigned NumCS = MFI->getCalleeSavedInfo().size();
      //if (NumCS == 0)
      //  AddDefaultPred(BuildMI(MBB, MBBI, dl, TII.get(ARM::tMOVr))
      //    .addReg(ARM::R3, RegState::Define)).addReg(ARM::SP);

      // Epilogue: load saved LR to R3 and branch off it.
      //AddDefaultPred(BuildMI(MBB, MBBI, dl, TII.get(ARM::tLDMIA)).addReg(ARM::R3)).addReg(ARM::R3, RegState::Define);
      MachineMemOperand *JTMMOLd =
        MF.getMachineMemOperand(MachinePointerInfo::getJumpTable(),
                               MachineMemOperand::MOLoad, 4, 4);
      AddDefaultPred(BuildMI(MBB, MBBI, dl, TII.get(ARM::tLDRi), ARM::R3)
        .addReg(ARM::R3, RegState::Kill)
        .addImm(0)
        .addMemOperand(JTMMOLd));

      // Insert checkpoint
      //BuildMI(MBB, MBBI, dl, TII.get(ARM::IDEM));
      
      unsigned StackDecrement;
      if(NumCS != 0)
        StackDecrement=4*MFI->getCalleeSavedInfo().size()+NumBytes;
      else
        StackDecrement=4+NumBytes;

      const TargetRegisterInfo *TRI = MBB.getParent()->getSubtarget().getRegisterInfo();
      if ( IdempotenceConstructionMode == IdempotenceOptions::OptimizeForSize ||
          MBB.computeRegisterLiveness(TRI, ARM::R1, MBBI, 1000) == MachineBasicBlock::LivenessQueryResult::LQR_Live )
      {
      AddDefaultPred(BuildMI(MBB, MBBI, dl, TII.get(ARM::tBL))).addExternalSymbol("_checkpoint_8");
      }else{
      emitThumbRegPlusImmediate(MBB, MBBI, dl, ARM::R2, ARM::SP, StackDecrement+ArgRegsSaveSize, TII, *RegInfo);

      AddDefaultPred(BuildMI(MBB, MBBI, dl, TII.get(ARM::tBL))).addExternalSymbol("_checkpoint_ret");
      }

      //// Decrement the stack pointer to reflect the CS regs being popped.
      //if(NumCS != 0)
      //  emitSPUpdate(MBB, MBBI, TII, dl, *RegInfo, 4*MFI->getCalleeSavedInfo().size()+NumBytes);
      //else
      //  emitSPUpdate(MBB, MBBI, TII, dl, *RegInfo, 4+NumBytes);
      emitSPUpdate(MBB, MBBI, TII, dl, *RegInfo, StackDecrement+ArgRegsSaveSize);

      // Restore it to where it thought it was
      MBBI = MBB.getLastNonDebugInstr();
    }else{
    ...
    if (AFI->getReturnRegsCount() > 3 && IdempotenceConstructionMode == IdempotenceOptions::NoConstruction) {
      AddDefaultPred(BuildMI(MBB, MBBI, dl, TII.get(ARM::tMOVr))
...
restoreCalleeSavedRegisters(MachineBasicBlock &MBB,
  ...
  if (IdempotenceConstructionMode != IdempotenceOptions::NoConstruction)
  {
    ...
    //MBB.insert(MI, AddDefaultPred(BuildMI(MF, DL, TII.get(ARM::tMOVr))
    //    .addReg(Reg, RegState::Define).addReg(ARM::SP)));
    MIB = BuildMI(MF, DL, TII.get(ARM::tLDMIA)).addReg(Reg);

  }else{
    MIB = BuildMI(MF, DL, TII.get(ARM::tPOP));
  }
  AddDefaultPred(MIB);
