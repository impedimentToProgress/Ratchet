...
void MachineVerifier::visitMachineInstrBefore(const MachineInstr *MI) {
  ...
  if (MI->getNumOperands() < MCID.getNumOperands() && IdempotenceConstructionMode == IdempotenceOptions::NoConstruction) {
  ...
  if (MO->readsReg()) {
    ...
    if (!regsLive.count(Reg)) {
      if (TargetRegisterInfo::isPhysicalRegister(Reg)) {
        ...
        if (Bad)//&& IdempotenceConstructionMode != IdempotenceOptions::OptimizeForSpeed)
